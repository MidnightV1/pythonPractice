...
def covertImageToAscii(fileName, cols, scale, moreLevels):
    ...
    # compute tile width
    w = W/cols
    # compute tile height based on the aspect ration and scale of the font
    h = w/scale
    # compute number of rows to use in the final grid
    rows = int(H/h)

    print("clos: %d, rows: %d" % (cols, rows))
    ...
    # an ASCII image is a list of charecter strings
    aimg = []
    ...
    for j in range(rows):
        ...
        for i in range(cols):
            ...
            # look up the ASCII character for grayscale calue (avg)
            if moreLevels:
